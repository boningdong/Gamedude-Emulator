...
extern uint8_t
  GRAM[LCD_WIDTH * LCD_HEIGHT]; // [!] Can be moved to external memory
...
void PPU_Tick() {
    switch (scanline) {
        case 0 ... 239:
            PPU_TickScanline(VISIBLE);
            break;
        case 240:
            PPU_TickScanline(POST);
            break;
        case 241:
            PPU_TickScanline(NMI);
            break;
        case 261:
            PPU_TickScanline(PRE);
            break;
    }
